...
{assign var="bVoteInfoEnabled" value=$LS->ACL_CheckSimpleAccessLevel($oConfig->GetValue('vote_state.blog.na_required_level'), $oUserCurrent, $oBlog, 'blog')}
...
<div class="blog-top">
	...
	<div id="vote_area_blog_{$oBlogId}" class="vote-topic 
		...
		<span class="vote-count" title="{$aLang.blog_vote_count}: {$iBlogCountVote}" data-count="{$iBlogCountVote}" id="vote_total_blog_{$oBlogId}" data-target_id="{$oBlogId}" data-target_type="blog">{$oBlogRating}</span>
